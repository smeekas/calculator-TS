...
cursor: not-allowed;`;
const enabledStyle: string = `cursor:pointer;color:black;`;
...
//  middle control
const memoryRecall = document.querySelector("#mr")! as HTMLDivElement;
const memoryClear = document.querySelector("#mc")! as HTMLDivElement;
const memoryPositive = document.querySelector("#mp")! as HTMLDivElement;
const memoryNegative = document.querySelector("#mn")! as HTMLDivElement;
const memoryStart = document.querySelector("#ms")! as HTMLDivElement;
...
memoryPositive.addEventListener("click", () => {
  ...
  const inputValue: number = eval(getInputValue());
  if (isNaN(inputValue)) {
...
memoryRecall.addEventListener("click", () => {
  ...
  input.textContent = numberFormat(
    (memoryPositiveValue - memoryNegativeValue).toString()
  );
  // }
});
...
function equal() {
  ...
  const answer: number = eval(inputValue);
...
function degreeRadian() {
  if (degreeToRadian) {
    ...
    input.textContent = numberFormat(
      ((eval(getInputValue()) * Math.PI) / 180).toString()
    );
...
function ExponentialDecimal() {
  ...
  const answer: number = eval(getInputValue());
  if (!input.textContent!.includes("e")) {
 ...
 */
function displayNumOp(number: string) {
  let inputValue: string = getInputValue();
...
const toExponential = (inputValue: number) => {
